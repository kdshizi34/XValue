...
func run(evm *EVM, contract *Contract, input []byte, readOnly bool) ([]byte, error) {   
	if contract.CodeAddr != nil {
		...
		if p := precompiles[*contract.CodeAddr]; p != nil {
			//return RunPrecompiledContract(p, input, contract)//----TODO----
			return RunPrecompiledContract(p, input, contract,evm)//+++++++++++TODO+++++++++++++
		}
	}
...
func (evm *EVM) Call(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int) (ret []byte, leftOverGas uint64, err error) { 
	...
	//if !evm.Context.CanTransfer(evm.StateDB, caller.Address(), value) {//----TODO----
	//if !bytes.Equal(AccountRef(addr).Address().Bytes(), types.DccpLockinPrecompileAddr.Bytes()) && !evm.Context.CanTransfer(evm.StateDB, caller.Address(), value) {//+++++++++TODO++++++++++
	if !types.IsDccpLockIn(input) && !types.IsDccpConfirmAddr(input) && !evm.Context.CanTransfer(evm.StateDB, caller.Address(), value) {//+++++++++TODO++++++++++
		log.Debug("=============evm.Call,ErrInsufficientBalance===============")//TODO
	...
	//bug
	if bytes.Equal(to.Address().Bytes(), types.DccpPrecompileAddr.Bytes()) {
	    str := string(input)
	    m := strings.Split(str,":")
	    if m[0] == "TRANSACTION" {
		value := m[2]
		...
		if strings.EqualFold(cointype,"ETH") == true || strings.EqualFold(cointype,"GUSD") == true || strings.EqualFold(cointype,"BNB") == true || strings.EqualFold(cointype,"MKR") == true || strings.EqualFold(cointype,"HT") == true || strings.EqualFold(cointype,"BNT") == true {
		    amount,_ := new(big.Int).SetString(value,10)
		     ret,err := evm.StateDB.GetDccpAccountBalance(caller.Address(),crypto.Keccak256Hash([]byte(strings.ToLower(cointype))),0)
		     if err == nil {
			if ret.Cmp(amount) < 0 {
			    log.Debug("=============evm.Call,TRANSACTION,ErrInsufficientBalance===============")//TODO
			    return nil, gas, ErrInsufficientBalance
			}
		     }
		}
		...
		if evm.BlockNumber.Cmp(num) > 0 {
		     ret,err := evm.StateDB.GetDccpAccountLockinHashkey(caller.Address(),crypto.Keccak256Hash([]byte(strings.ToLower(m[3]))),0)
		    log.Debug("========EVM.call,","ret",ret,"","============")//TODO
	...
	//if !evm.StateDB.Exist(addr) {//-----TODO-----
	if !bytes.Equal(AccountRef(addr).Address().Bytes(), types.DccpPrecompileAddr.Bytes()) && !evm.StateDB.Exist(addr) { //+++++++++TODO++++++++++++

	    log.Debug("===========EVM.call,is not exist","addr",addr.Hex(),"","============") //TODO
		precompiles := PrecompiledContractsHomestead
		if evm.ChainConfig().IsByzantium(evm.BlockNumber) {
			precompiles = PrecompiledContractsByzantium
		}
	...
	if !bytes.Equal(to.Address().Bytes(), types.DccpPrecompileAddr.Bytes()) {//+++++++TODO+++++++++
	    //log.Debug("=============evm.Call,Transfer===============")//TODO
