...
// PublicXvcAPI is the collection of xvc API methods exposed over
// both secure and unsecure RPC channels.
type PublicXvcAPI struct {
	node *Node // Node interfaced by this API
}

// NewPublicXvcAPI creates a new API definition for the public xvc methods
// of the node itself.
func NewPublicXvcAPI(node *Node) *PublicXvcAPI {
	return &PublicXvcAPI{node: node}
}

// Peers retrieves all the information we know about each individual peer at the
// protocol granularity.
func (api *PublicXvcAPI) Peers() ([]*p2p.PeerInfo, error) {
	server := api.node.Server()
	if server == nil {
		return nil, ErrNodeStopped
	}
...
// protocol granularity.
func (api *PublicXvcAPI) NodeInfo() (*p2p.NodeInfo, error) {
	server := api.node.Server()
	if server == nil {
		return nil, ErrNodeStopped
	}
...
func (api *PublicXvcAPI) Datadir() string {
