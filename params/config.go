...
var (
	// XvcChainConfig contains the chain parameters to run a node on the xvalue network.
	XvcChainConfig = &ChainConfig{
	...
	// AllCcdexProtocolChanges contains every protocol change (EIPs) introduced
	// and accepted by the Ethereum core developers into the Ccdex consensus.
	//
	// This configuration is intentionally not using keyed fields to force anyone
	// adding flags to the config to also have to set these fields.
	...
	AllCcdexProtocolChanges = &ChainConfig{big.NewInt(1337), big.NewInt(0), nil, false, big.NewInt(0), common.Hash{}, big.NewInt(0), big.NewInt(0), big.NewInt(0), nil, nil, nil, nil, &CcdexConfig{Period: 0, Epoch: 30000}}
...
type ChainConfig struct {
	...
	Ccdex *CcdexConfig `json:"ccdex,omitempty"`
}
...
// CcdexConfig is the consensus engine configs for proof-of-authority based sealing.
type CcdexConfig struct {
	Period uint64 `json:"period"` // Number of seconds between blocks to enforce
	Epoch  uint64 `json:"epoch"`  // Epoch length to reset votes and checkpoint
}

// String implements the stringer interface, returning the consensus engine details.
func (c *CcdexConfig) String() string {
	return "ccdex"
}
