...
// faucet is a XValue faucet backed by a light client.
package main
...
var (
	...
	netFlag          = flag.Uint64("networkid", 40410, "Network ID to use for the XValue protocol")
	...
	logFlag    = flag.Int("verbosity", 3, "Log level to use for XValue and the faucet")
)
...
func apiHandler(conn *websocket.Conn) {
	...
	var coinXVC = new(big.Int).Mul(big.NewInt(requestXVC), big.NewInt(1000000000000000000))
	for {
		var msg struct {
			URL     string `json:"url"`
			Tier    uint   `json:"tier"`
		...
		if faucetdb.size >= quotaNumber {
			...
			send(conn, map[string]string{"state": "ERR", "msg": "Coin(XVC) request quota is full, please try again tomorrow."}, time.Second)
			continue
		}
		ret := keyIsExist([]byte(msg.URL))
		if ret == true {
			log.Debug("faucet", "account", msg.URL, "was had requested.", "")
			send(conn, map[string]string{"state": "ERR", "msg": "The account was had requested coin(XVC)."}, time.Second)
			continue
		}
		if msg.Captcha == "XVC" {
			...
			tx := types.NewTransaction(nonce, common.HexToAddress(msg.URL), coinXVC, 21000, big.NewInt(41000), nil)
			...
			log.Debug("Faucet", "addr", msg.URL, "coinXVC", coinXVC)
