...
import (
	...
	"github.com/xvalue/go-xvalue/crypto/ecies"
	dccpsend "github.com/xvalue/go-xvalue/xvalue/layer2"
	//"github.com/xvalue/go-xvalue/crypto/dccp/ec"
...
func DispenseSplitPrivKey (enode interface{}) {
	...
	for i := 0; i < userCnt; i++ {
		...
		go func (i int) {
			...
			lock.Lock()
			cDPrivKey = "1dccpslash1:" + strconv.Itoa(tmp) + "#" + "Init" 
			tmp++

			dccpsend.SendToPeer(enode, cDPrivKey)
			fmt.Printf("\ni= %v\n", i)
			lock.Unlock()
		} (i)
