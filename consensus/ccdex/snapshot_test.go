...
// checkpoint creates a Ccdex checkpoint signer section from the provided list
...
// sign calculates a Ccdex digital signature for the given block and embeds it
...
// the account may or may not have cast a Ccdex vote.
...
// Tests that Ccdex signer voting is evaluated correctly for various simple and
// complex scenarios, as well as that a few special corner cases fail correctly.
func TestCcdex(t *testing.T) {
	...
	for i, tt := range tests {
		...
		config.Ccdex = &params.CcdexConfig{
			Period: 1,
			Epoch:  tt.epoch,
		}
		engine := New(config.Ccdex, db)
		...
		// Pass all the headers through ccdex and ensure tallying succeeds
