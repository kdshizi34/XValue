...
/*func main () {
	//获取dccp地址
	dccpAddress := "0xA8dC61209400C9A23bf1fe625c2919c3626Bc157"
	toAddressHex := "0x7b5Ec4975b5fB2AA06CB60D0187563481bcb6140"
	amount, _ := new(big.Int).SetString("1",10)
	gasLimit := uint64(0)
	tx, txhash, err := Erc20_newUnsignedTransaction(client, dccpAddress, toAddressHex, amount, nil, gasLimit, "BNT")
	if err != nil {
		fmt.Printf("%v\n",err)
		return
	}
...
func Erc20_newUnsignedTransaction (client *ethclient.Client, dccpAddress string, toAddressHex string, amount *big.Int, gasPrice *big.Int, gasLimit uint64, tokenType string) (*types.Transaction, *common.Hash, error) {
	...
	fromAddress := common.HexToAddress(dccpAddress)
