...
import (
	...
	"github.com/xvalue/go-xvalue/common/mclock"
	...
	"github.com/xvalue/go-xvalue/light"
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light XValue protocol")
	...
	log.Info("Light XValue protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light XValue peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light XValue handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light XValue peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light XValue message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light XValue message arrived", "code", msg.Code, "bytes", msg.Size)
